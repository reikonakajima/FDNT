"""
fdnt_g08.py : copied from GalSim/examples/demo5.py (Demo #5).  No star images!
...
import logging
import time
#sys.path.append('/home/reiko/local/python')
import galsim
...
def main(argv):
    ...
    Make images similar to that done for the Great08 challenge:
      ...
      - Each postage stamp is 130 x 130 pixels (big, to accomodate FDNT)
    ...
    Take the first argument as gal_resolution.
    """
    ...
    if len(sys.argv) == 1 or sys.argv[1] not in resdict.keys():
        print 'first argument is key to gal_resolution: "20" = 2.0, "04" = 0.4, etc.'
        sys.exit(1)

    res_text = sys.argv[1]
    print 'starting', res_text, '...'
    ...
    gal_file_name = os.path.join('output','g08_gal_R%s.fits' % (res_text))
    gal_signal_to_noise = 200       # Great08 "LowNoise" run
    gal_resolution = resdict[sys.argv[1]]        # r_gal / r_psf (use r = half_light_radius)
    ...
    gal_ellip_rms = 0.8             # using "shear" definition of ellipticity.
    gal_ellip_max = 0.95            #
    gal_g1 = 0.013                  # check multiplicative bias
    gal_g2 = 0.000                  # check additive bias
    ...
    logger.info('    - PSF ellip = (%.3f,%.3f)',psf_g1,psf_g2)
    ...
    #psf.applyShear(g1=psf_e1,g2=psf_e2)  # DEBUG
