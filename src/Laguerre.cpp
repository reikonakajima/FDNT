...
LVector::basis(DMatrix& out,
  ...
  for (int ilo=0; ilo<npts; ilo+=BLOCKING_FACTOR) {
    ...
    tmv::MatrixView<double> outRange = out.rowRange(ilo, ihi);
...
LVector::design(DMatrix& out,
  ...
  for (int ilo=0; ilo<npts; ilo+=BLOCKING_FACTOR) {
    int ihi = MIN(npts, ilo + BLOCKING_FACTOR);
    tmv::ConstVectorView<double> invsigRange = invsig.subVector(ilo,ihi);
    tmv::MatrixView<double> outRange = out.rowRange(ilo,ihi);
...
LVector::kBasis(const DVector& kxunit, const DVector& kyunit,
  ...
  for (int ilo=0; ilo<npts; ilo+=BLOCKING_FACTOR) {
    int ihi = MIN(npts, ilo + BLOCKING_FACTOR);
    tmv::MatrixView<double> realRange = kReal->rowRange(ilo,ihi);
    tmv::MatrixView<double> imagRange = kImag->rowRange(ilo,ihi);
