     ...
     char *argv[])
{
  ...
  try {
    ...
    // Start up our top-level random number generators
    UniformDeviate uMaster;
    ...
    long seed = static_cast<long>(numeric_limits<long>::max() * uMaster);

#pragma omp parallel for schedule(dynamic)
    for (int iTheta = 0.; iTheta<nTheta; iTheta++) { // each iTheta is a parallel-processing chunk
      ...
      UniformDeviate u(seed + iTheta);
      GaussianDeviate g(u);
      ostringstream oss, osserr;

      int nFailTheta = 0;	// Number of failures at this angle
      
#ifdef _OPENMP
      osserr << "Thread " << omp_get_thread_num() << " working on iTheta " << iTheta << endl;
      cerr << osserr.str();
#endif
