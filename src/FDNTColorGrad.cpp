// Run FDNT on color gradient files from E. Semboloni with SExtractor input files
     ...
     char *argv[])
{
  ...
  try {
    ...
    // Read the PSF
    ColorGradGaussian cpsf(psfName);
    SBAdd& psf = cpsf.getSBProfile();
    double ee50psf = EnclosedFluxRadius(psf);
    Ellipse psfBasis(0., 0., log(ee50psf/1.17));

    PSFInformation psfinfo(psf, psfBasis);
