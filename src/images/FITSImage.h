// $Id: FITSImage.h,v 1.24 2005/09/22 17:38:09 garyb Exp $
...
namespace img {
  ...
  protected:
    ...
    static  list<const FITSImageBase*> imgQ;
    typedef list<const FITSImageBase*>::iterator fptr;
  ...
  template <class T=float>
    class FITSImage: public FITSImageBase {
      public:
      FITSImage(const string fname, 
		const FITS::Flags f=FITS::ReadOnly,
		const int HDUnum=1);
      FITSImage(const string fname, 
		const FITS::Flags f,
      ...
      string filename() const {return parent.getFilename();}
      ...
      FITS::DataType getNativeType() const {return nativeType;}
      ...
      void renameExtension(const string newext);
    ...
    private:
      ...
      FITSImage(const FITSImage& rhs):parent(0) {}
      ...
      mutable list<Image<T> > imgList;
      typedef typename list<Image<T> >::iterator imgptr;
      ...
      //Close out the data buffer.  Returns true if there are Images
      // still attached.
