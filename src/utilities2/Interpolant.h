// $Id: Interpolant.h,v 1.4 2012/01/06 19:54:55 garyb Exp $
// Class used to interpolate in 1d or 2d
...
inline double sinc(double x) {
  if (std::abs(x)<0.001) return 1.- PI*PI*x*x/6.;
...
public:
  ...
  double urange() const {return 1./(PI*tolerance);}
...
public:
  ...
  ~SincInterpolant() {}
  double getTolerance() const {return tolerance;}
  double xrange() const {return 1./(PI*tolerance);}
...
public:
  ...
  double urange() const {return sqrt(1./tolerance)/PI;}
  double xval(double x) const {
    x=abs(x);
    if (x>1.) return 0.;
    else return 1.-x;
...
public:
 Lanczos(int n_, bool fluxConserve_=false, double tol=1e-3): 
  ...
  double xval(double x) const { 
    ...
    if (fluxConserve) retval *= 1 + 2.*u1*(1-cos(2*PI*x));
    return retval;
  }
  double uval(double u) const {
    u = std::abs(u);
